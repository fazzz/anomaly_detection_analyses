...
ndist_4=`wc -l r-r_pairs_union-open-close.txt | awk '{print $1}'`

if [ ${ndist} != ${ndist_4} ]; then
...
if [ ! -d anomaly_d_r-r_4 ]; then
    mkdir -p anomaly_d_r-r_4
fi
...
for r in ${sparsity[*]}; do
  ...
  assl2 -Sparsity ${r} -ndim ${ndist} \
                             ../dist_r-r_4/r-r_dist_union-open-close.txt \
      ../../../close/analyses_1/dist_r-r_4/r-r_dist_union-open-close.txt \
      > a_d_r-r_sp=${r}.txt
