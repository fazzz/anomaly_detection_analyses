...
if [ ! -d dist_r-r_coff=6.0_ij-min=10 ]; then
    mkdir dist_r-r_coff=6.0_ij-min=10
fi

cd dist_r-r_coff=6.0_ij-min=10
...
if [ ! -d anomaly_d_r-r_coff=6.0_ij-min=10 ]; then
    mkdir -p anomaly_d_r-r_coff=6.0_ij-min=10
fi

cd anomaly_d_r-r_coff=6.0_ij-min=10
...
for r in ${sparsity[*]}; do
  ...
  assl2 -Sparsity \${r} -ndim ${ndist} \\
                             ../dist_r-r_coff=6.0_ij-min=10/r-r_dist_union-open-close.txt \\
      ../../../close/analyses_2/dist_r-r_coff=6.0_ij-min=10/r-r_dist_union-open-close.txt \\
      > a_d_r-r_sp=\${r}.txt
