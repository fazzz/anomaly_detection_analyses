...
for s in ${sparsity[*]}; do
    ...
    i=1
    cat ../anomaly_d_r-r/a_d_r-r_sp=${s}_ss_A_res-res_pairs.txt | while read line; do
...
EOF
    ...
    i=1
    cat ../anomaly_d_r-r/a_d_r-r_sp=${s}_ss_B_res-res_pairs.txt | while read line; do
	echo ${line} | awk '{printf("%4d %4d\n%4d %4d\n", $1, $2, $3, $4)}' > tmp/${i}_B
...
EOF
    ...
    cat tmp01 >> ss-of-cov_d_r-r_sp=${s}.gp
    cat tmp02 >> ss-of-cov_d_r-r_sp=${s}.gp
...
quit
EOF
    gnuplot ss-of-cov_d_r-r_sp=${s}.gp
